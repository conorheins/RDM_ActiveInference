function [MDP] = spm_MDP_VB_X_distribO(MDP,OPTIONS)
...
if length(MDP) > 1
    ...
    for i = 1:length(MDP)
        ...
        % solve this trial
        %------------------------------------------------------------------
        OUT(i) = spm_MDP_VB_X_distribO(MDP(i),OPTS);
...
for g = 1:Ng
    ...
    % assume constant preferences over time, if only final states are specified
    %----------------------------------------------------------------------
    if size(Vo{g},2) == 1
...
for t = 1:T
    ...
    for f = 1:Nf
        try
            ...
            s(f,t) = find(rand < cumsum(ps),1); % state has the index of each state of the D hidden factors
        end
    end
    ...
    for g = 1:Ng
        try
            o(g,t) = MDP.o(g,t);
            O{g,t} = full(sparse(o(g,t),1,1,No(g),1));
        catch
            O{g,t} = MDP.GP{g}(:,ind{:});
            [~,o(g,t)] = max(O{g,t});
        end
    ...
    if isfield(MDP,'link')
        ...
        %------------------------------------------------------------------
        MDP.mdp(t) = spm_MDP_VB_X_distribO(mdp);
    ...
    tstart = tic;
    for f = 1:Nf
        x{f} = spm_softmax(spm_log(x{f})/length(x)); % divide by four because there are four hidden factors in x?
    end
    ...
    for k = p
        for j = 1:S
            ...
            %--------------------------------------------------------------
            Q(k) = Q(k) + spm_MDP_G(A,xq); % (original version, for 5 dimensional A matrix)
