function MDP = DEM_demo_MDP_dotnew
...
fprintf('Time taken to run a single trial: %.2f seconds\n',toc);
...
tic
MDP = spm_MDP_VB_X(MDP);
fprintf('Time taken to run a %d trials: %.2f minutes',N,toc/60);

save(sprintf('MDPresults_%s.mat',datestr(now,'mmddyy')),'MDP')
