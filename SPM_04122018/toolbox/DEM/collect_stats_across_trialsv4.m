...
function sub_matrix = collect_stats_across_trialsv4(data_dir,all_files,file_idx)

load(fullfile(data_dir,all_files{file_idx}));
num_trials = size(MDPresult,2);

% statistics to collect:
% 1. noise value (precision parameter that scales a distribution before softmaxing, between 0 and 16 -- proxy for sensory uncertainty/incoherence)
...
%   encountered
% 13. Time until categorization
% 14 - 21. 
% 22. Mean time taken until Occam's window is reached, averaged over fixations


sub_matrix = zeros(num_trials,14);
...
for trial_i = 1:num_trials
    ...
    if isempty(time2choose)
        sub_matrix(trial_i,13) = size(current_trial.o,2);
    else
        sub_matrix(trial_i,13) = time2choose;
    end
    
    %% 14 - 21: trajectory of expected free energy of policies over time
    sub_matrix(trial_i,14:21) = mean(current_trial.EV(2:5,:),1);
    
    %% 22. Mean time taken until Occam's window is reached, averaged over fixations
    % (N.B. average calculated only for saccades to quadrants with RDMs in
    % them)
     ...
     sub_matrix(trial_i,22) = mean(time2bound);
