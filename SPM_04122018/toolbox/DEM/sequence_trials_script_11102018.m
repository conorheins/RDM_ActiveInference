...
%     N = 10; % debugging
    ...
    save(sprintf('C:/RDPsearch_Hierarch5_results/RDPsearch_Hierarch5_%d_%s.mat',iter,datestr(now,'ddmmyy')),'MDPresult','all_configs',...
...
%% inspect some trials with the video function

[fnam] = uigetfile('*.mat');
load(fullfile('RDPsearch_Hierarch3_results',fnam))

% subset_to_inspect = 1:size(MDPresult,2); % debugging

subset_to_inspect = randi(size(MDPresult,2),20,1);
for i = 1:length(subset_to_inspect)
    fprintf('Prior Belief: %s, Strength: %.2f, Sensory Uncert: %.2f\n',...
        prior_scene_beliefs{subset_to_inspect(i)},... 
        prior_scene_probs(subset_to_inspect(i)),...
        noise_vals(subset_to_inspect(i))); 
    MDP_beliefs_video_ENIPC(MDPresult,subset_to_inspect(i),all_configs); 
    pause; 
    close gcf; 
end
