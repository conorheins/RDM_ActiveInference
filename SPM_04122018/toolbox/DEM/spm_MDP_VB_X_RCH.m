function [MDP] = spm_MDP_VB_X_RCH(MDP,OPTIONS)
% active inference and learning using variational message passing
% FORMAT [MDP] = spm_MDP_VB_X_RCH(MDP,OPTIONS)
%
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_RCH(MDP(:,i),OPTIONS);
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_RCH(mdp);
        ...
        if ~HMM || T == t
            ...
            if Np(m) > 1
                for k = p{m}
                    ...
                    for j = t:S
                        ...
                        %--------------------------------------------------
                        EV_temp = spm_MDP_G(A(m,:),xq(m,:));
                        EV(k) = EV(k) + EV_temp;
                        Q(k) = Q(k) + EV_temp; % so maybe if you wanted to do just expected utility, you'd comment out this line (gets rid of epistemic value)
                        % also maybe you should track the evolution of spm_MDP_G(A(m,:),xq(m,:)) over time and policies
                        for g = 1:Ng(m)
            ...
            if isfield(MDP,'factor')
                ...
                %----------------------------------------------------------
%                 fprintf('Free Energy on iteration %d: %.4f\n',t,sum(H(:)));
                MDP(m).H_int(1,t) = sum(H(:));
                
                
%                 if sum(H(:)) > - chi
% %                     fprintf('Occams Window Met!\n')
%                     T = t;
%                 end

                % RCH edit Jan 29 2019
                if t > 1 && MDP(m).u(2,t-1) == 2 % if sampled action is to break (index 2), then end the loop
                    T = t;
                end
