...
% Karl Friston
% $Id: FEP_self_entropy.m 7179 2017-10-02 18:13:03Z karl $
...
% self organisation in terms of self entropy
%==========================================================================

% Sample image of blanket states for the i-th blanket
%--------------------------------------------------------------------------
W     = 256;
X0    = ones(W,1);
it    = 0:256:1024;
for i = 1:size(S,2)
    ...
    for tt = 1:4
        ...
        % canonical variate analysis for dimension reduction
        %------------------------------------------------------------------
        CVA = spm_cva(b,e,X0);
        w   = CVA.w;
        ...
        %------------------------------------------------------------------
        D     = 0;
        for j = 1:size(CVA.w,2)
            d    = CVA.w(:,j)*ones(1,W);
            D    = D + (d' - d).^2;
        end
        
        % get imaging blanket states
        %------------------------------------------------------------------
        bb    = b;
        for j = 1:W
            d       = D(:,j);
        ...
        % evaluate relative entropy is
        %------------------------------------------------------------------
        Hbbe = spm_logdet(Cbbe)/2;
        Hbb  = spm_logdet(Cbb)/2;
        Hb   = spm_logdet(Cb)/2;
        ...
        % accumulate and plot
        %------------------------------------------------------------------
        subplot(3,4,tt), imagesc(Cbb), axis square
        
        subplot(3,1,2), bar([mean(HB,1) mean(HBb,1) mean(IB,1)])
        drawnow
