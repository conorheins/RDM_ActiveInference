...
function sub_matrix = collect_stats_across_trialsv3(data_dir,all_files,file_idx)
...
%   encountered
% 13 - 20. The epistemic value, averaged over policies going to the four
% quadrants, for every timestep
% 21. Time until categorization



sub_matrix = zeros(num_trials,21);
...
for trial_i = 1:num_trials
    ...
    sub_matrix(trial_i,13:20) = mean(current_trial.EV(2:5,:),1);
    time2choose = find(current_trial.o(3,:) > 5,1)-1;
    if isempty(time2choose)
        sub_matrix(trial_i,21) = size(current_trial.o,2);
    else
        sub_matrix(trial_i,21) = time2choose;
    end
