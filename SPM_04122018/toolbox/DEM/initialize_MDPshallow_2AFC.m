function [ MDPshallow ] = initialize_MDPshallow_2AFC(p,T,policy_depth)
%INITIALIZE_MDPSHALLOW_2AFC Initialize lower/shallower level of MDP, with an
...
%                       sensory uncertainty into the mapping between true RDP motion (LEFT OR RIGHT) 
...
D{1} = [ 1 1 1]'; % Direction: {'null','LEFT','RIGHT'}
...
for f1 = 1:Ns(1)
    ...
    elseif f1 == 2 % 'LEFT'
        a{1}(2:3,f1) = power(512,10)*spm_softmax([1;0]*p);       
    elseif f1 == 3 % 'RIGHT'
        a{1}(2:3,f1) = power(512,10)*spm_softmax([0;1]*p);
    end 
end
