function MDP = DEM_demo_MDP_dot2
...
% slow probably because of my clunky recursive implementation of the
% surprise
% tic
% MDP = spm_MDP_VB_X(MDP);
% toc

% best way is to stick with the old version
tic
MDP = spm_MDP_VB_X_old(mdp);
toc

% doesn't work (perhaps because of un-updated version of spm_dot
% (line 683 in new VB_X)
% tic
% MDP = spm_MDP_VB_X_new(MDP);
% toc
...
MDP = spm_MDP_VB_X_old(MDP);
toc
save('MDPresults_02102018.mat','MDP')
...
result_table = zeros(1000,4);
for i = 1:N
    result_table(i,1) = MDP(i).s(1,1);
    o      = MDP(i).o(1,:);                       % outcomes
    result_table(i,2) = double(any(o == 6) & ~any(o == 7));  % accuracy
    result_table(i,3) = find([(o > 5), 1],1) - 1;            % number of saccades
    result_table(i,4) = mean(MDP(i).rt);                     % reaction time
end
